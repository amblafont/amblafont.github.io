		...
		var retMerge = function (movedNodes) {
			if (movedNodes.b && (!movedNodes.b.b)) {
				...
				if (_v4.b && (!_v4.b.b)) {
					var i = _v4.a;
					return {
						graph: $author$project$Polygraph$removeLoops(
							A3($author$project$Polygraph$merge, i, n.id, model.graph)),
						valid: true
					};
				} else {
...
var $author$project$Geometry$determine_label_position = F9(
	function (length, angle, edge_width, start, end, curve, label_position, label_alignement, label_size) {
		var bezier = A4(
			...
			_Utils_Tuple2(0, 0),
			length,
			A2($elm$core$Debug$log, 'curve', curve),
			angle);
...
var $author$project$Main$helpMsg = function (model) {
	...
	switch (_v0.$) {
		...
		case 'CutHead':
			return msg('Mode cut arrow.' + (' [RET] or [click] to confirm. [ESC] to cancel. ' + '[c] to switch between head/tail.'));
		case 'RenameMode':
